...
def test_fee_conversion_handles_missing_fee_currency(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    now = time.time()
    now_ms = int(now * 1000)
    ...
    ledger_path = tmp_path / "executions_missing.testnet.jsonl"
    with ledger_path.open("w", encoding="utf-8") as fh:
    ...
    inventory, layer_state = spot_pnl_module.spot_inventory_and_pnl(
        events=[dict(event) for event in base_events], return_layers=True
    )
    spot_state = inventory.get("BTCUSDT")
    assert spot_state is not None
    assert spot_state["realized_pnl"] == pytest.approx(-2.0)
    ...
    summary = pnl_module._build_daily_summary([dict(event) for event in base_events])
    day_key = time.strftime("%Y-%m-%d", time.gmtime(now))
    day_bucket = summary.get(day_key, {})
    symbol_bucket = day_bucket.get("BTCUSDT", {})
    assert symbol_bucket.get("spot_fees") == pytest.approx(2.0)
    ...
    summary_path = tmp_path / "pnl_daily_missing.json"
    summary_path.parent.mkdir(parents=True, exist_ok=True)
    monkeypatch.setattr(pnl_module, "_SUMMARY", summary_path)
    monkeypatch.setattr(
        pnl_module,
        "read_ledger",
        lambda *_, **__: [dict(event) for event in base_events],
    )
    pnl_module.invalidate_daily_pnl_cache()
    ...
    inferred_path = tmp_path / "executions_inferred.testnet.jsonl"
    monkeypatch.setattr(
        pnl_module,
        "_ledger_path_for",
        lambda *_, **__: inferred_path,
    )
    ...
    for idx, base_event in enumerate(base_events):
        ...
        payload["orderId"] = f"missing-{idx}"
        payload["execTime"] = now_ms + idx
        pnl_module.add_execution(payload)

    stored_events: list[dict[str, object]] = []
    with inferred_path.open("r", encoding="utf-8") as fh:
        for line in fh:
            if not line.strip():
                continue
            stored_events.append(json.loads(line))

    assert stored_events
    assert all(event.get("feeCurrency") == "BTC" for event in stored_events)
