...
def test_guardian_summary_flags_low_confidence_under_defaults(tmp_path: Path) -> None:
    ...
    assert "52.00%" in reason_text
    assert summary["thresholds"]["buy_probability_pct"] == 52.0
...
def test_guardian_watchlist_and_scorecard(tmp_path: Path) -> None:
    ...
    assert scorecard["buy_threshold"] == 52.0
    assert scorecard["sell_threshold"] == 42.0
    assert scorecard["exit_buy_threshold"] == pytest.approx(53.5, rel=1e-3)
    assert scorecard["exit_sell_threshold"] == pytest.approx(40.5, rel=1e-3)
    assert scorecard["configured_buy_threshold"] == 52.0
