...
FastAPI-бэкенд (`bybit_app/backend/app.py`) закрывает чувствительные эндпоинты `/orders/*`, `/kill-switch/*` и `/state/*` обязательной аутентификацией. Чтобы включить доступ для клиентов:

1. Установите общий секрет через переменную окружения или `.env`:

   ```env
   BACKEND_AUTH_TOKEN=super-secret-token
   ```

   Переменная читается из `Settings`, поэтому её можно задать и через UI настроек.
...
PY
   ...
   curl -X POST http://localhost:8000/orders/place \
     ...
     -H "X-Bybit-Timestamp: $TIMESTAMP" \
     -H "X-Bybit-Signature: $SIGNATURE" \
     -d "$BODY"
   ```

Неверные или отсутствующие заголовки приводят к ответу `401/403`, поэтому убедитесь, что автоматизация или прокси всегда прокидывает токен.
